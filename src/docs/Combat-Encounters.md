...
+ Disposition (editable; "Match token", Freindly, Neutral, Hostile, Secret)
...
While in active combat, all movement will be tracked and displayed as movement history, if a combatant is hovered in the combat tracker or on the canvas. These will automatically deleted when a combout encounter is ended.
...
Both the `Base` and the `Squad` combatant groups rely on enemy NPC actors having their organoization set in their actor sheet.
