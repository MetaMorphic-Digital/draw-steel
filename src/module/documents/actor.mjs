...
export default class DrawSteelActor extends BaseDocumentMixin(foundry.documents.Actor) {
  ...
  async updateFromCompendium({ uuid, skipDialog } = {}) {
    const sourceActor = await fromUuid(uuid ?? this._stats.compendiumSource);

    if (!sourceActor) throw new Error("Failed to find the source document!");

    if (!skipDialog) {
      const content = document.createElement("div");

      content.insertAdjacentHTML("afterbegin", `<p>${
        game.i18n.localize("DRAW_STEEL.SOURCE.CompendiumSource.UpdateFrom.ContentActor")
      }</p>`);
    ...
    await this.update({ "==system": sourceActor.toObject().system });

    const effectCreateData = [];
    const effectUpdateData = [];
    const effectDeleteIds = [];

    for (const effect of sourceActor.effects) {
      /** @type {ActiveEffectData} */
      const effectData = effect.toObject();
      const currentEffect = this.effects.get(effect.id);
      if (currentEffect) effectUpdateData.push({
        _id: effect.id,
      ...
      else effectCreateData.push(effectData);
    }
    for (const effect of this.effects) if (!sourceActor.effects.get(effect.id)) effectDeleteIds.push(effect.id);

    await this.updateEmbeddedDocuments("ActiveEffect", effectUpdateData);
    await this.createEmbeddedDocuments("ActiveEffect", effectCreateData, { keepId: true });
    await this.deleteEmbeddedDocuments("ActiveEffect", effectDeleteIds);

    const itemCreateData = [];

    for (const item of sourceActor.items) {
      const localItem = this.items.get(item.id);

      if (localItem) {
        await localItem.updateFromCompendium({ uuid: item.uuid, skipDialog: true });
      }
      else {
        itemCreateData.push(game.items.fromCompendium(item));
      }
    }

    await this.createEmbeddedDocuments("Item", itemCreateData, { keepId: true });
  }
